...
export default function BlogPage() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return blogPosts.filter((post) => {
      const categoryValue = `${post.category} ${post.categoryLabel}`.toLowerCase();
      const categoryMatch =
        activeCategory === "All" ||
      ...
      const searchMatch =
        ...
        post.categoryLabel.toLowerCase().includes(normalizedSearch);
  ...
  const featuredArticle = pagePosts[0];
  const articles = pagePosts.slice(1);
